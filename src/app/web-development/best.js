...
const ContentItem = ({ title, description, isRightAligned }) => {
  return (
    <div className={` ${isRightAligned ? "text-right" : "text-left"} mb-10`}>
      <div
        className={`w-full   ${isRightAligned ? "text-right" : "text-left"}`}
      >
        <h3
          className={`text-[24px] leading-[24px] ${monsterfont3.className} text-white my-[40px] mx-[30px] `}
        >
          {title}
        </h3>
        <p className="text-[15px] leading-[22px]  my-[40px] mx-[30px] text-semi-white font-helveticaneue">
          {description}
        </p>
      </div>
    </div>
...
const AccordionItem = ({ title, content, isActive, onClick, isInView }) => {
  return (
    <div className="relative">
      ...
      >
        <span
          className={`text-[21px] leading-[22px] ${monsterfont2.className} text-white `}
        >
          {title}
        </span>
        {isActive ? (
  ...
  const accordionData = [
    ...
    },
    {
      ...
      content: [
        ...
        {
          title: (
            <Link href="/seo-consulting" className="hover:text-blue-400">
      ...
      <div className="max-w-[1200px] mx-auto p-4">
        <div className="text-center mb-16 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <h2
              className={` ${styles["description"]} ${monsterfont1.className} mt-6 md:mt-[100px] text-white`}
            >
              PRACTICES FOR
            </h2>
            <h2
              className={` ${styles["description"]} ${monsterfont1.className} text-white`}
            >
              WEB DEVELOPMENT
            </h2>
          </div>
