      ...
      <div className="max-w-[1200px] mx-auto" ref={scalableRef}>
        <div className="text-center mb-16 relative overflow-hidden">
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <span
              ref={scalableRef}
              className={`mb-8 md:text-[100px] text-[40px] transform transition-transform duration-700 ${
                monsterfont3.className
              } text-[#151515] ${
                isInView
          ...
          <div className="relative z-10">
            <h2
              className={` ${style["description"]} ${monsterfont1.className} mt-14 text-white`}
            >
              FRANCHISE MARKETING
            </h2>
            <h2
              className={`${style["description"]} ${monsterfont3.className} text-white -mt-2`}
            >
              SERVICES
            </h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 ">
          {services.map((service, index) => (
            <div
              ...
              } h-fit relative group rounded-lg p-7 transition-all duration-700  ${
                styles["box-color"]
              } transform
              ${
                isInView
            ...
            >
              <div
                className={`absolute top-4 right-4 text-blue-500 font-helveticaneue ${styles["number"]} `}
              >
                {service.id}
              </div>

              <h3
                className={`${styles["video-title"]} text-white font-helveticaneue font-[500]  transition-colors duration-300`}
              >
                {service.title}
              </h3>
              <div className="mb-6">
