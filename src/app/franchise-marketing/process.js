  ...
  return (
    <div className="min-h-screen bg-black text-white flex items-center justify-center p-6">
      <div className="w-full max-w-6xl mx-auto p-4">
        {/* Title on top */}
        <h1 className={`${styles["our-text"]} ${monsterfont1.className}`}>
          Our
        </h1>
        <h2
          className={`text-[35px] leading-[40px] md:text-[56px] tracking-[0.06em] md:leading-[60px] md:${styles["pr-text"]} ${monsterfont.className} text-center `}
        >
        ...
        <div className="flex justify-between mb-8 max-w-5xl mx-auto ">
          {steps.map((step) => (
            <button
              key={step.number}
              className={`${
                activeStep === step.number ? " text-white" : "bg-transparent "
              } py-2 transition-colors text-sm ${styles["step-text"]} ${
        ...
        <div className="mt-[100px] space-y-4 text-white  max-w-5xl mx-auto">
          <div className="">
            <h2 className={`${styles["pr-title"]} ${monsterfont2.className}`}>
              {steps[activeStep - 1].title}
            </h2>
            <p
              className={`${styles["pr-description"]} font-opensans text-semi-white`}
            >
