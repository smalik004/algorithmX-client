    ...
    items: [
      ...
      { name: 'Ebook', href: '/ebooks', icon: 'fa-image' },
      {
        ...
        subItems: [
          ...
          { name: 'Product Audits', href: '/product-audits', icon: 'fa-keyboard' },
        ]
      },
...
export default function Navbar() {
  ...
  return (
    ...
    >
      ...
      <div
        className={`bg-blue-600 text-white py-2 px-4 text-center transition-all duration-300 ${showAnnouncement ? 'opacity-100 h-auto' : 'opacity-0 h-0 py-0 overflow-hidden absolute'
          }`}
      >
      ...
      {/* Navbar */}
      <nav className={`fixed top-0 left-0 right-0 z-20 transition-all duration-300 ${scrolled ? 'bg-black' : 'bg-transparent'
                ...
                {Object.keys(menuData).map((menuName) => (
                  ...
                  >
                    {menuData[menuName].type === 'link' ? (
                      <Link
                        href={menuData[menuName].href}
                        className={`px-2 py-2 text-[14px] font-helveticaneue font-medium transition-colors duration-200 ${activeMenu === menuName ? 'text-white' : 'text-[#FFFFFFCC] hover:text-white'
                          }`}
                      >
                        {menuName}
                      </Link>
                      ...
                      <div
                        className={`px-2 py-2 text-[14px] font-helveticaneue font-medium cursor-pointer transition-colors duration-200 ${activeMenu === menuName && isOpen ? 'text-white' : 'text-[#FFFFFFCC] hover:text-white'
                          }`}
                      >
                        {menuName}
                      </div>
                          ...
                          {menuData[menuName].items
                            ...
                            .map((item) => (
                              ...
                              >
                                ...
                                <span
                                  className={`text-base font-medium block cursor-pointer transition-colors duration-200 relative ${activeSubMenu === item.name
                                    ? 'text-blue-300'
                          ...
                          return (
                            <div className="flex h-full">
                              {columns.map((columnItems, colIndex) => (
                                <div
                                  key={colIndex}
                                  className={`flex-1 flex flex-col divide-y divide-gray-800/30 h-full ${colIndex !== columns.length - 1 ? 'border-r border-gray-800/30' : ''
                                    }`}
                                >
                      ...
                      {(!activeSubMenu || (menuName === "Creative Assets" && !menuData[menuName].items.find(item => item.name === activeSubMenu)?.subItems)) && (
                        <div className="flex flex-col items-center justify-center h-full p-8 text-center">
                          <div className="w-16 h-16 mb-4 text-teal-400/80">
                            <i className={`text-4xl drop-shadow-[0_0_2px_rgba(45,212,191,0.3)] ${menuName === "Creative Assets" ? "fas fa-palette" : ""
                              }`}></i>
                          </div>
