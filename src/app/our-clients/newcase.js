...
// Transform API data to grid items format
const transformAPIDataToGridItems = (apiData) => {
  ...
  const uniqueItems = apiData.filter(
    (item, index, self) => index === self.findIndex((t) => t.id === item.id)
  );

  return uniqueItems.map((item) => {
    // Extract metrics from the metrices object
    const metrics = [];
    if (item.metrices) {
      // Add conversion rate
      if (item.metrices.conversionRate) {
        metrics.push({
          label: item.metrices.conversionRate,
          description: "Conversion Rate",
        });
      }

      // Add average rating
      if (item.metrices.avgRatings) {
        metrics.push({
          label: item.metrices.avgRatings.toString(),
          description: "Average Rating",
        });
      }

      // Add total orders
      if (item.metrices.totalOrders) {
        metrics.push({
          label: item.metrices.totalOrders,
    ...
    return {
      ...
      bgColor: brandColor, // Use the RGB color directly
      hoverContent:
        item.brandAboutDesc ||
        item.solutionTitle ||
        "Project description not available",
      timeRead: "5 min",
      logoImage: item.brandLogoURL || null,
      characterImage: item.brandImageURL || null,
      link: `/case-study/${item.id}`,
      industry: item.brandIndustry || "Other",
      region: item.brandType || "Global",
      services: item.brandServices || "Consulting",
      metrics:
        metrics.length > 0
          ? metrics
          : [{ label: "N/A", description: "No metrics available" }],
      // Additional data that might be useful
      productType: item.productType,
      brandRGB: item.brandRGB,
...
const getUniqueValues = (array, key) => {
  return [
    ...new Set(array.filter((item) => item[key]).map((item) => item[key])),
  ];
};
...
const PortfolioGrid = ({ data = [] }) => {
  ...
  // Transform API data when component mounts or data changes
  useEffect(() => {
    if (data && data.length > 0) {
      const transformedItems = transformAPIDataToGridItems(data);
      setGridItems(transformedItems);
  ...
  const industries = useMemo(
    () => ["Show All", ...getUniqueValues(gridItems, "industry")],
    [gridItems]
  );
  const services = useMemo(
    () => ["Show All", ...getUniqueValues(gridItems, "services")],
    [gridItems]
  );
  const regions = useMemo(
    () => ["Show All", ...getUniqueValues(gridItems, "region")],
    [gridItems]
  );
      ...
      <div className="container mx-auto max-w-[1200px]">
        ...
        {filteredItems.length === 0 && (
          <div className="text-white text-center py-12">
            <p className="text-lg mb-2">No items match your current filters</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
          {filteredItems.map((item, index) => (
            <Link key={item.id} href={item.link} className="block">
              ...
              >
                <div
                  className={`absolute inset-0 rounded-xl text-white shadow-lg transition-all duration-150 ease-out ${
                    hoveredIndex === index
                      ? "shadow-2xl border-[2px] border-white"
                      : ""
                  }`}
                  style={{
                    backgroundColor: item.bgColor, // Apply the brand RGB color directly
                    height: hoveredIndex === index ? "400px" : "144px",
                    padding: hoveredIndex === index ? "16px" : "12px",
                    top: hoveredIndex === index ? "-70px" : "0",
                    transitionProperty: "all",
                    transitionDuration:
                      hoveredIndex === index ? "120ms" : "300ms",
                ...
                >
                  ...
                  <div
                    className={`h-full flex flex-col items-center justify-center text-center transition-all duration-200 ${
                      hoveredIndex === index
                  ...
                  >
                    ...
                    <h3 className="text-xs md:text-sm lg:text-base font-medium px-2 md:px-4">
                      {item.title}
                    </h3>
                  </div>
                  ...
                  <div
                    className={`absolute inset-0 p-3 md:p-6 flex flex-col items-center transition-all duration-300 ${
                      hoveredIndex === index
                        ? "opacity-100"
                        : "opacity-0 pointer-events-none"
                    }`}
                  >
                    ...
                    <div className="w-10 h-10 md:w-12 md:h-12 mt-1 md:mt-0 mb-2 md:mb-4">
                      {item.logoImage ? (
                        ...
                        <div
                          className="w-full h-full bg-white/20 rounded-lg flex items-center justify-center transition-all duration-300 ease-out"
                          style={{
                            transform:
                              hoveredIndex === index
                                ? "translateY(0)"
                    ...
                    <div className="w-20 h-20 md:w-28 md:h-28 mb-3 md:mb-6 flex items-center justify-center relative">
                      ...
                      {item.characterImage ? (
                        ...
                        <div
                          className="w-16 h-16 md:w-20 md:h-20 bg-white/30 rounded-full flex items-center justify-center transition-all duration-300 ease-out relative z-10"
                          style={{
                            transform:
                              hoveredIndex === index
                                ? "scale(1)"
